...
import { LayoutDashboard, Store, Mic, Landmark, Map as MapIcon, Sparkles, Zap, Crown, Star, TrendingUp } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={clsx(
        ...
        {/* Navigation Pill with Premium Effects */}
        <div className="relative w-full max-w-[420px] h-[5rem] pointer-events-auto group/nav animate-[nav-float-gentle_4s_ease-in-out_infinite]">
          ...
          <div className="absolute inset-0 overflow-visible pointer-events-none">
            {[...Array(10)].map((_, i) => (
              <div
                key={i}
                className="absolute w-1 h-1 rounded-full bg-gradient-to-br from-emerald-300 to-cyan-400"
                style={{
                  top: `${10 + Math.random() * 80}%`,
                  left: `${10 + Math.random() * 80}%`,
                  animation: `star-twinkle-float ${3 + Math.random() * 3}s ease-in-out infinite`,
          ...
          <div className="absolute inset-0 animate-[nav-glow-pulse_4s_ease-in-out_infinite]">
             <svg viewBox="0 0 420 80" className="w-full h-full" preserveAspectRatio="none">
                <defs>
                    ...
                    <linearGradient id="navBorderGlow" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stopColor="rgba(16, 185, 129, 0.3)">
                          <animate attributeName="stop-color" values="rgba(16, 185, 129, 0.3);rgba(6, 182, 212, 0.5);rgba(16, 185, 129, 0.3)" dur="3s" repeatCount="indefinite" />
                        </stop>
                        <stop offset="50%" stopColor="rgba(6, 182, 212, 0.6)">
                          <animate attributeName="stop-color" values="rgba(6, 182, 212, 0.6);rgba(16, 185, 129, 0.8);rgba(6, 182, 212, 0.6)" dur="3s" repeatCount="indefinite" />
                        </stop>
                ...
                {/* Main Shape with Enhanced Curve */}
                <path 
                ...
                <path 
                    d="M0,28 Q0,0 28,0 L150,0 C165,0 175,10 185,22 Q200,48 210,48 Q220,48 235,22 C245,10 255,0 270,0 L392,0 Q420,0 420,28 L420,80 L0,80 Z" 
                    fill="url(#navRadialDepth)" 
                />
                
                {/* Top Highlight Line */}
                <path 
                    d="M28,2 L150,2 C165,2 175,12 185,24 Q200,50 210,50 Q220,50 235,24 C245,12 255,2 270,2 L392,2" 
                ...
                {navItems.slice(0, 2).map((item, idx) => {
                    ...
                    return (
                        <button 
                            ...
                            className="relative flex flex-col items-center justify-center gap-1.5 w-16 h-full group/item active:scale-90 transition-all duration-300"
                        >
                            ...
                            <div className={clsx(
                                ...
                                isActive 
                                    ? `bg-gradient-to-br ${item.gradient} bg-opacity-15 border-${item.color}-400/40 shadow-[0_0_20px_currentColor] animate-[icon-pop-bounce_0.6s_ease-out] animate-[icon-glow-pulse_2s_ease-in-out_infinite]`
                                    : "bg-white/5 border-transparent text-slate-500 group-hover/item:bg-white/10 group-hover/item:border-white/20 group-hover/item:text-slate-300"
                            )}>
                                {/* Active Inner Glow */}
                                {isActive && (
                                    <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient} opacity-15 blur-lg`}></div>
                                )}
                                
                                {/* Top Shine */}
                                {isActive && (
                                    <div className="absolute top-0 inset-x-2 h-[2px] bg-gradient-to-r from-transparent via-white/60 to-transparent rounded-full blur-[1px]"></div>
                                )}
                                ...
                                <item.icon 
                                    ...
                                    className={clsx(
                                        ...
                                        isActive && `text-${item.color}-400 drop-shadow-[0_0_10px_currentColor]`
                                    )}
                                />
                                ...
                                {isActive && (
                                    <>
                                        <div className="absolute top-1 right-1 w-1 h-1 rounded-full bg-white/50 blur-[1px] animate-pulse"></div>
                                        <div className="absolute bottom-1 left-1 w-1 h-1 rounded-full bg-white/40 blur-[1px] animate-pulse" style={{ animationDelay: '0.5s' }}></div>
                                    </>
                                )}
                            </div>
                            ...
                            <span className={clsx(
                                ...
                                isActive 
                                    ? `text-${item.color}-400 scale-105 animate-[label-slide-fade_0.4s_ease-out]` 
                                ...
                                {/* Active Underline */}
                                {isActive && (
                                    <div className={`absolute -bottom-0.5 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-${item.color}-400 to-transparent rounded-full shadow-[0_0_8px_currentColor]`}></div>
                                )}
                            </span>
                            ...
                            {isActive && (
                                <Star 
                                    size={10} 
                                    className={`absolute -top-1 -right-1 text-${item.color}-300 animate-pulse`}
                                    fill="currentColor"
                                />
                            )}
                        </button>
             ...
             >
                ...
                {[...Array(4)].map((_, i) => (
                  <div
                    ...
                    style={{
                      animation: `fab-orbit-particle ${8 + i * 2}s linear infinite`,
                ...
                <div className="relative w-full h-full rounded-[1.5rem] bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-600 flex items-center justify-center animate-[fab-glow-intense_3s_ease-in-out_infinite] group-active/fab:scale-90 transition-all duration-300 border-[4px] border-[#020617] overflow-hidden">
                    ...
                    <div className="absolute inset-0 opacity-40">
                        <div className="absolute inset-0 bg-gradient-to-r from-emerald-300/50 via-cyan-300/50 to-emerald-300/50 bg-[length:200%_200%] animate-[gradient-wave_4s_ease_infinite] blur-md"></div>
                    </div>
                    ...
                    <Mic 
                        size={32} 
                        className={clsx(
                            "relative z-30 text-white drop-shadow-[0_3px_12px_rgba(0,0,0,0.6)] transition-all duration-300",
                            view === 'VOICE_ASSISTANT' && "animate-[fab-float-smooth_3s_ease-in-out_infinite]"
                        )}
                        strokeWidth={3}
                    />
