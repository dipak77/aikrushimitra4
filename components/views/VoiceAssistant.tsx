    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col h-[100dvh] w-full overflow-hidden" ref={containerRef}>
       ...
       <div className="absolute inset-0">
            ...
            <div className="absolute inset-0 opacity-20" 
                 style={{backgroundImage: 'linear-gradient(rgba(16, 185, 129, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(16, 185, 129, 0.1) 1px, transparent 1px)', backgroundSize: '40px 40px'}}></div>
       ...
       <div className="absolute top-0 left-0 right-0 p-4 pt-safe-top flex justify-between items-center z-[220] bg-gradient-to-b from-[#020617]/90 via-[#020617]/50 to-transparent">
          <button 
             onClick={handleBack} 
          ...
          <div className={clsx("px-4 py-1.5 rounded-full border backdrop-blur-xl transition-all duration-500 shadow-2xl", 
             status === 'connected' ? "border-emerald-400/50 bg-emerald-500/20 text-emerald-100" : 
             status === 'error' ? "border-red-400/50 bg-red-500/20 text-red-200" : 
             status === 'offline' ? "border-yellow-400/50 bg-yellow-500/20 text-yellow-200" :
             "border-emerald-500/20 bg-white/5 text-slate-300"
          )}>
             <span className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                 {status === 'connected' && <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_12px_#34d399]"/>}
       ...
       <div className="flex-1 flex flex-col items-center w-full relative z-10 pt-2">
                  ...
                  {status === 'idle' ? (
                    ...
                    <div className="flex flex-col items-center">
                      <div className="w-24 h-24 rounded-full bg-gradient-to-br from-emerald-500/20 to-cyan-600/10 backdrop-blur-md border-2 border-emerald-400/50 flex items-center justify-center mb-5 shadow-[0_0_60px_rgba(16,185,129,0.5)] relative overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-tr from-emerald-500/20 via-transparent to-cyan-500/20 animate-spin" style={{ animationDuration: '8s' }} />
          ...
          <div className="mt-8 w-full px-6 flex flex-col items-center z-40">
              <h2 className="text-2xl font-black text-emerald-100 tracking-tight mb-3 drop-shadow-[0_2px_30px_rgba(16,185,129,0.7)]">
                 {status === 'connected' ? "मी ऐकतोय..." : status === 'idle' ? t.voice_title : status === 'error' ? "Connection Error" : status === 'offline' ? 'No Internet' : 'Connecting...'}
              </h2>
              ...
              {(status === 'idle' || (status === 'connected' && transcripts.length < 2)) && (
                 <div className="w-full max-w-[320px] flex flex-col gap-3 mt-7">
                    {t.voice_hints.slice(0, 3).map((hint: string, i: number) => (
                        <div key={i} className="w-full px-5 py-3.5 rounded-2xl bg-gradient-to-br from-emerald-500/15 to-cyan-500/8 border border-emerald-400/25 backdrop-blur-xl text-sm font-medium text-emerald-50 shadow-2xl flex items-center gap-3 hover:from-emerald-500/20 hover:to-cyan-500/12 transition-all cursor-pointer active:scale-[0.97]">
       ...
       <div className={clsx("absolute bottom-0 inset-x-0 bg-gradient-to-t from-black via-black/95 to-transparent pt-20 pb-safe-bottom px-6 z-20 transition-all duration-700 flex flex-col justify-end", 
           transcripts.length === 0 ? "translate-y-full opacity-0" : "translate-y-0 opacity-100"
       )} style={{ minHeight: '32vh', maxHeight: '45vh' }}>
           <div className="flex flex-col gap-3.5 max-h-full overflow-y-auto pb-5" style={{
                maskImage: 'linear-gradient(to bottom, transparent, black 5%, black 95%, transparent)',
                WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 5%, black 95%, transparent)',
           }}>
                {transcripts.slice(-6).map((msg, i) => (
                   <div key={i} className={clsx("p-4 rounded-2xl backdrop-blur-xl border max-w-[85%] text-sm font-medium shadow-2xl", 
                       msg.role === 'user' ? "self-end bg-gradient-to-br from-emerald-500/30 to-emerald-600/20 text-emerald-50 border-emerald-400/40 rounded-tr-md shadow-[0_4px_20px_rgba(16,185,129,0.3)]" : "self-start bg-gradient-to-br from-white/15 to-white/8 text-slate-100 border-white/20 rounded-tl-md"
                   )}>
                       {msg.text}
                   </div>
                ))}
