    ...
    return (
        <div className="h-full w-full overflow-y-auto overflow-x-hidden hide-scrollbar bg-transparent text-slate-100 lg:pl-24 selection:bg-yellow-500/30 flex flex-col relative">
            ...
            <style>{`
                ...
                @keyframes orbPulse { 0%, 100% { transform: scale(1); opacity: 0.7; } 50% { transform: scale(1.1); opacity: 0.9; } }
            `}</style>
            ...
            <div className="pt-4 px-6 pb-2 flex items-center justify-between z-50 gap-4">
                 ...
                 <div className="flex items-center gap-2">
                     ...
                     <button 
                        ...
                        className="w-10 h-10 rounded-full bg-slate-900/50 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white hover:border-emerald-500/50 transition-all"
                     >
                        <Shield size={18} />
                     </button>
            ...
            <div className="px-6 grid grid-cols-1 md:grid-cols-12 gap-5 pb-32 max-w-7xl mx-auto w-full">
                ...
                <div className="col-span-1 md:col-span-9 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <FeatureCard icon={ScanLine} title={t.quick_action_doctor} variant="disease" onClick={() => onNavigate('DISEASE_DETECTOR')} delay={100} />
                    <FeatureCard icon={FlaskConical} title={t.quick_action_soil} variant="soil" onClick={() => onNavigate('SOIL')} delay={150} />
                    <FeatureCard icon={TrendingUp} title={t.menu_yield} variant="yield" onClick={() => onNavigate('YIELD')} delay={200} />
                    <FeatureCard icon={MapIcon} title={t.menu_area} variant="area" onClick={() => onNavigate('AREA_CALCULATOR')} delay={250} />
                </div>
                <div className="col-span-1 md:col-span-6 animate-enter" style={{animationDelay: '300ms'}}>
                     <IllustrativeBanner title={t.govt_schemes} subtitle={txt.subsidies} icon={Landmark} gradient="from-emerald-800 to-teal-900" pattern="coins" onClick={() => onNavigate('SCHEMES')} />
                </div>
                <div className="col-span-1 md:col-span-6 animate-enter" style={{animationDelay: '350ms'}}>
                     <IllustrativeBanner title={txt.crop_doctor_title} subtitle={txt.crop_doctor_sub} icon={Leaf} gradient="from-indigo-900 to-blue-900" pattern="scan" onClick={() => onNavigate('DISEASE_DETECTOR')} />
                </div>
            </div>
        </div>
    );
